...
const withAndroidTransparentNavigation: ConfigPlugin = config => {
  return withAndroidStyles(config, config => {
    ...
    const appThemeStyle = styles?.resources?.style?.find((style: any) => style.$.name === 'AppTheme')

    if (appThemeStyle && appThemeStyle.item) {
      // Check if the translucent navigation item already exists
      const existingItem = appThemeStyle.item.find((item: any) => item.$.name === 'android:windowTranslucentNavigation')
